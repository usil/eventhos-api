...
_main() {
	...
	if [ "$1" = 'mysqld' ] && ! _mysql_want_help "$@"; then
		...
		if [ -z "$DATABASE_ALREADY_EXISTS" ]; then
            ...
            if [ "$ENABLE_MULTIPLE_USERS" == "true" ]; then
				mysql_note "multiple users flag is enabled"
                # create several users
                while IFS='=' read -r name value ; do
                    if [[ $name =~ ^MYSQL_USER_[0-9]+$ ]]; then
                        username=$value
                        
                        password_env_key="$name""_PASSWORD"        
                        password=$(printf '%s\n' "${!password_env_key}")

                        database_name_env_key="$name""_DATABASE"        
                        database_name=$(printf '%s\n' "${!database_name_env_key}")

                        if [ -n "$username" ] && [ -n "$password" ] && [ -n "$database_name" ]; then
                            create_user_and_database $username $password $database_name    
                            database_dump_folder_env_key="$name""_DUMP_FOLDER"        
                            database_dump_folder=$(printf '%s\n' "${!database_name_env_key}")        

                            if [ -n "$database_dump_folder" ]; then
                                # since docker_process_init_files don't allow database name as param, uses MYSQL_DATABASE
                                # so we need to override it with custom database name 
                                MYSQL_DATABASE=$database_name
                                docker_process_init_files /docker-entrypoint-initdb.d/$database_dump_folder/*
                            fi                                                    
                        fi
                ...
                # load the dump using default $MYSQL_DATABASE $MYSQL_USER
...
create_user_and_database() {
	...
	mysql_note "Creating user:$_MYSQL_USER password:**** database:$_MYSQL_DATABASE"

	# Creates a custom database and user if specified
	if [ -n "$_MYSQL_DATABASE" ]; then
		mysql_note "Creating database ${_MYSQL_DATABASE}"
		docker_process_sql --database=mysql <<<"CREATE DATABASE IF NOT EXISTS \`$_MYSQL_DATABASE\` ;"
	fi

	if [ -n "$_MYSQL_USER" ] && [ -n "$_MYSQL_PASSWORD" ]; then
		mysql_note "Creating user ${_MYSQL_USER}"
		docker_process_sql --database=mysql <<<"CREATE USER '$_MYSQL_USER'@'%' IDENTIFIED BY '$_MYSQL_PASSWORD' ;"

		if [ -n "$_MYSQL_DATABASE" ]; then
			mysql_note "Giving user ${_MYSQL_USER} access to schema ${_MYSQL_DATABASE}"
			docker_process_sql --database=mysql <<<"GRANT ALL ON \`${_MYSQL_DATABASE//_/\\_}\`.* TO '$_MYSQL_USER'@'%' ;"
		fi
	fi
}
